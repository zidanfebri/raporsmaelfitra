...
<div class="row mb-4">
    <div class="col">
        <h4 class="fw-bold"><i class="bi bi-grid-3x3-gap-fill me-2 text-primary"></i>Buku Leger Nilai Ujian Murni (UTS / UAS)</h4>
        <p class="text-muted">Kompilasi murni nilai lembar hasil ujian siswa tanpa pengaruh pembobotan nilai harian.</p>
    </div>
</div>

<div class="card border-0 shadow-sm mb-4 bg-light">
    <div class="card-body p-3">
        <form action="{{ route('wali.leger_ujian') }}" method="GET" class="row g-2 align-items-end">
            <div class="col-md-2">
                <label class="small fw-bold text-muted mb-1">Jenis Ujian</label>
                <select name="jenis_ujian" class="form-select form-select-sm" onchange="this.form.submit()">
                    <option value="uts" {{ request('jenis_ujian') == 'uts' ? 'selected' : '' }}>UTS (Ujian Tengah Semester)</option>
                    <option value="uas" {{ request('jenis_ujian') == 'uas' ? 'selected' : '' }}>UAS (Ujian Akhir Semester)</option>
                </select>
            </div>
            <div class="col-md-2">
            ...
            @endif
            <div class="col-md-2">
                <a href="{{ route('wali.leger_ujian') }}" class="btn btn-sm btn-outline-secondary w-100">Reset Filter</a>
            </div>
        </form>
    </div>
</div>
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white pt-3 border-0">
        <h6 class="fw-bold m-0 text-primary"><i class="bi bi-table me-1"></i> Data Leger Ujian {{ strtoupper($jenis_ujian) }} Kelas: {{ $infoKelas->nama_kelas }}</h6>
    </div>
            ...
            <table class="table table-bordered table-hover align-middle mb-0 text-center small" style="font-size:12px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $row['nisn'] }}</td>
                        @foreach($allMapelsSorted as $m)
                            @php $score = $row['scores'][$m->id] ?? 0; @endphp
                            <td class="fw-bold {{ $score == 0 ? 'text-muted opacity-50' : 'text-secondary' }}">{{ $score > 0 ? $score : '-' }}</td>
                        @endforeach
...
@elseif($kelas_id)
    <div class="alert alert-warning text-center">Belum ada data nilai ujian terkalkulasi pada kombinasi filter ini.</div>
@else
    <div class="alert alert-info text-center">Silakan tentukan filter kelas dan parameter tahun ajaran untuk melihat lembar buku leger ujian murni.</div>
@endif
@endsection